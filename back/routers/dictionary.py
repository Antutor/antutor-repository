...
async def get_all_dictionary_terms():
    translated_terms = []
    response = supabase.table("concepts").select("concept_id").execute()
    for row in response.data:
        translated = await translate_en_to_ko(row["concept_id"])
...
async def get_dictionary_term(term: str):
    ...
    for row in concepts:
        if row["concept_id"].lower() == term.lower():
            target_concept = row
            break
    ...
    if not target_concept:
        for row in concepts:
            translated_k = await translate_en_to_ko(row["concept_id"])
    ...
    term_str = await translate_en_to_ko(target_concept["concept_id"])
    def_str = await translate_en_to_ko(target_concept.get("dict_content") or "")
